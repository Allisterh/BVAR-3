...
#detach(package:BVAR,unload=TRUE) #* Make sure to detach BVAR-package if active to avoid conflicts
library(bvartools) 
library(lubridate)
library(dplyr)
library(stringr)
...
General government expenditures (as share of GDP)
```{r}
...
Read data on CLIFS index, downloaded from ECB's database
```{r}
...
Get initial values for the VAR model
```{r}
y <- t(data$data$Y)
x <- t(data$data$Z)
...
Define priors, adjust to the choice of data
```{r}
#* Set priors for the coefficients
a_mu_prior <- matrix(0, m) #* Vector of prior parameter means
a_v_i_prior <- diag(1, m) #* Inverse of the prior covariance matrix, i.e. prior precision
...
Define conditions for the Gibbs sampler
```{r}
...
Create storage for the posterior draws
```{r}
...
Summary statistics for posterior draws of the coefficients
```{r}
...
Model summary
```{r}
summary(bvar_est) #* Summary for covariance matrix and coefficients
plot(bvar_est) #* Visual presentation of the posterior draws, by the graphs it seems to have converged nicely
```

Define orthogonal impulse response functions
```{r}
par(mfrow=c(3,3))  
...
## Expenditure model (Alternative specification)
We define a alternative specification where we swap government debt with government expenditure for robustness.
We use the same priors and procedure as in the first model but they may be adjusted depending on choice of variable:
